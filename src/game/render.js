...
// [stated] 넘어짐이 **다른 자세보다 너무 컸다**(폭 32px, 서있기 13px 의 2.4배) → 1.5 → **1.2**
const SOC_SCALE = { 8: 0.65, 9: 0.67, 10: 0.66, 11: 0.67, 12: 1.2 };
...
export function createRenderer(canvas){
  ...
  function drawBurst(x, y, team, t){     // t: 0(피격 순간) -> 1(소멸)
    ...
    // [stated] 가운데 **흰 4x4 사각형은 없앤다.** 기본 캐릭터는 피격 순간 통째로 하얘져서
    // 이게 묻혀 안 보였는데, **스킨은 하얘지지 않아 사각형만 튀어 보였다**.
    // 기본에서도 원래 안 보이던 것이라 지워도 달라지는 게 없다
  }
  ...
  function drawPlayer(p, i, xOverride, yOverride, blind = 0, tick = 0, colorOf = null, off = false, skin = 0){
    ...
    if (ARENA.soccer && isReady(soccerImg)){
      ...
      // [stated] **좌우 태클 모션이 또 같았다.** 옆모습(서기·뛰기)은 왼쪽 그림뿐이라 오른쪽이면
      // 뒤집어 그리는데, **태클은 좌(10)·우(11) 그림이 따로 있는데도 똑같이 뒤집어서** 도로 같아졌다.
      // → 뒤집기는 **왼쪽 그림만 있는 자세**(서기·뛰기)에만 쓴다
      const mirror = face === 3 && (p.tkl | 0) === 0 && (p.stun | 0) === 0;
