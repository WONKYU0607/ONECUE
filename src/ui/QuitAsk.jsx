...
export default function QuitAsk({ pvp, exit, onQuit, onStay }){
  return (
    <div className="modal-back" onClick={e => e.target === e.currentTarget && onStay?.()}>
      <div className="modal ask quit-ask">
        <p className="ask-t">{t(exit ? 'quit.appTitle' : 'quit.title')}</p>
        <p className="ask-d">{t(exit ? 'quit.appDesc' : (pvp ? 'quit.pvp' : 'quit.ai'))}</p>
