...
try {
  const A = await open('sid=SA&uid=userA&nick=A&mode=create&n=2');
  await wait(400);
  const B = await open(`sid=SB&uid=userB&nick=B&mode=join&code=${A.code}`);
  await wait(600);
  const got = [];
  for (let round = 1; round <= 2; round++){
    A.send(JSON.stringify({ t: 'start' })); await wait(400);
