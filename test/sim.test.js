...
// 시작은 양쪽 '설치 완료'로 자동. 실제로는 아이템을 전부 놓아야 완료할 수 있으므로
// 테스트에서는 준비 상태를 직접 세팅한다 (그 규칙 자체는 items.test.js에서 검사)
const IN = (f0 = 0, f1 = 0) => [{ dx:0, dy:0, fire:f0, ready:0, place:null, thr:null },
                                { dx:0, dy:0, fire:f1, ready:0, place:null, thr:null }];
const markReady = s => { s.ready = [true, true]; };
...
console.log('페이즈 전이');
{
  ...
  markReady(s); step(s, IN()); assert(s.phase === PH_COUNT && s.timer === CD_TICKS, 'START -> 카운트다운');
...
{
  const s = newState(); markReady(s); step(s, IN());
